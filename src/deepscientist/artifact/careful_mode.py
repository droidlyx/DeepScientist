"""Opt-in careful-mode switch for process-discipline enforcement.

When ``startup_contract.careful_mode`` is set to ``{"mode": "on"}`` on a
...
- Rationalization / pilot / stale-state reminders are appended to the
  system prompt from ``src/prompts/careful_mode_addendum.md``.
...
When ``careful_mode.mode == "off"`` (the default), none of the above
...
CAREFUL_MODE_MODES = {"off", "on"}


def coerce_careful_mode(value: Any, *, field_name: str = "careful_mode") -> dict[str, Any]:
    """Normalize a user-supplied ``careful_mode`` value.
    ...
    if isinstance(value, str):
        ...
        if mode not in CAREFUL_MODE_MODES:
            raise ValueError(
                f"`{field_name}` must be one of {sorted(CAREFUL_MODE_MODES)!r}."
            )
        return {"mode": mode}
    ...
    if mode not in CAREFUL_MODE_MODES:
        raise ValueError(
            f"`{field_name}.mode` must be one of {sorted(CAREFUL_MODE_MODES)!r}."
        )
    return {"mode": mode}


def read_careful_mode(quest_root: Path) -> dict[str, Any]:
    """Read a normalized ``careful_mode`` dict from the quest's startup_contract.

    Returns ``{"mode": "off"}`` whenever the field is absent, malformed,
    or explicitly disabled. This is the authoritative gate for the
    careful-mode behaviors; callers should not try to re-derive it from
    other quest fields.
    """
    ...
    if not isinstance(startup_contract, dict):
        return {"mode": "off"}
    raw = startup_contract.get("careful_mode")
...
def is_careful_mode_on(quest_root: Path | None) -> bool:
    """Convenience: ``True`` iff the quest's careful_mode is explicitly on."""

    if quest_root is None:
        return False
    try:
        return read_careful_mode(quest_root).get("mode") == "on"
    except Exception:
        return False
...
def _is_agent_flagged_regression(record: dict[str, Any]) -> bool:
    ...
    intentionally-negative ablations. Careful mode's regression routing should
...
def _regression_guidance_override(
    ...
    return {
        ...
        "summary": (
            "Main run was marked refuted. Careful mode routes through "
            "`analysis-campaign` to isolate the cause before the next full run."
        ),
        "why_now": (
            "The agent has already judged the main run as refuted. Careful mode "
...
def maybe_inject_careful_mode_regression_routing(
    quest_root: Path,
    record: dict[str, Any],
    ...
    Only fires when all three hold:
      - the quest's ``careful_mode.mode`` is ``"on"``
