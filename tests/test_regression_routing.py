...
from deepscientist.artifact.strict_mode import (
    _is_agent_flagged_regression,
    coerce_strict_mode,
    is_strict_mode_on,
    maybe_inject_strict_mode_regression_routing,
    read_strict_mode,
)


def _write_quest_yaml(quest_root: Path, strict_mode: dict | None) -> None:
    quest_root.mkdir(parents=True, exist_ok=True)
    payload: dict = {"quest_id": "test", "startup_contract": {}}
    if strict_mode is not None:
        payload["startup_contract"]["strict_mode"] = strict_mode
...
def test_numeric_negative_delta_alone_is_not_a_regression() -> None:
    # Intentionally conservative: strict mode does NOT auto-classify from
...
# ---- strict_mode config helper ----


def test_strict_mode_defaults_to_off_when_missing(tmp_path: Path) -> None:
    _write_quest_yaml(tmp_path, None)
    assert read_strict_mode(tmp_path)["mode"] == "off"
    assert is_strict_mode_on(tmp_path) is False


def test_strict_mode_accepts_structured_on(tmp_path: Path) -> None:
    _write_quest_yaml(tmp_path, {"mode": "on"})
    assert read_strict_mode(tmp_path)["mode"] == "on"
    assert is_strict_mode_on(tmp_path) is True


def test_strict_mode_accepts_plain_string_on(tmp_path: Path) -> None:
    _write_quest_yaml(tmp_path, "on")
    assert read_strict_mode(tmp_path)["mode"] == "on"
    assert is_strict_mode_on(tmp_path) is True


def test_strict_mode_malformed_value_falls_back_to_off(tmp_path: Path) -> None:
    _write_quest_yaml(tmp_path, {"mode": "loud"})
    assert read_strict_mode(tmp_path)["mode"] == "off"


def test_coerce_strict_mode_rejects_garbage() -> None:
    for bad in [None, 0, [], {"mode": "foo"}, {"other": "on"}, 3.14]:
        try:
            coerce_strict_mode(bad)
        except ValueError:
            continue
        raise AssertionError(f"coerce_strict_mode should have rejected {bad!r}")


def test_coerce_strict_mode_accepts_structured_and_string() -> None:
    assert coerce_strict_mode({"mode": "on"}) == {"mode": "on"}
    assert coerce_strict_mode("off") == {"mode": "off"}


# ---- routing override: gated on strict_mode AND agent-flagged refuted ----


def test_override_not_applied_when_strict_mode_off(tmp_path: Path) -> None:
    _write_quest_yaml(tmp_path, {"mode": "off"})
    record = _run_record(verdict="refuted")
    base = build_guidance_for_record(record)
    out = maybe_inject_strict_mode_regression_routing(tmp_path, record, base)
    # Strict mode off -> guidance must be untouched.
    assert out is base
...
def test_default_main_run_guidance_is_unchanged_without_strict_mode(
    tmp_path: Path,
) -> None:
    # Without touching strict_mode, build_guidance_for_record should keep
